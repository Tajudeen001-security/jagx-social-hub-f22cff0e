...
import { ArrowLeft, Send, Users, Plus, Image, X, UserPlus, Reply, Paperclip, BarChart3, FileText, Download, UserMinus } from "lucide-react";
...
import GroupPollBlock from "@/components/GroupPollBlock";
import GroupPollCreator from "@/components/GroupPollCreator";
...
const GroupChatPage = () => {
  ...
  const removeMember = async (targetUserId: string, targetUsername: string) => {
    if (!groupId || !user) return;
    if (targetUserId === groupInfo?.creator_id) {
    ...
    await supabase.from("group_messages" as any).insert({
      group_id: groupId, user_id: user.id, sender_id: user.id,
      content: `👋 ${targetUsername} was removed from the group`, message_type: "system",
    });
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const path = `${user.id}/${groupId}/${Date.now()}_${safe}`;
      ...
      const { error: insErr } = await supabase.from("group_messages" as any).insert({
        group_id: groupId, user_id: user.id, sender_id: user.id,
        content: file.name, message_type: "file",
        attachment_url: publicUrl, attachment_name: file.name,
        attachment_size: file.size, attachment_mime: file.type,
      });
  ...
  const handlePollCreated = async (pollId: string, question: string) => {
    if (!user || !groupId) return;
    await supabase.from("group_messages" as any).insert({
      group_id: groupId, user_id: user.id, sender_id: user.id,
      content: question, message_type: "poll", poll_id: pollId,
    });
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
          ...
          if (isSystem) {
            return (
              <div key={msg.id} className="flex justify-center">
                <div className="px-3 py-1 rounded-full bg-surface/60 border border-border/30 text-[11px] text-muted-foreground">
                  {msg.content}
                </div>
              </div>
            );
            ...
            >
              <div className={`max-w-[80%] ${isMine ? "" : "flex gap-2"}`}>
                ...
                <div>
                  ...
                  } ${msg.message_type !== "text" ? "p-1" : "px-4 py-2.5"} ${isPoll || isFile ? "min-w-[260px]" : ""}`}>
                    {hasReply && (
                    ...
                    {msg.message_type === "image" ? <img src={msg.content} className="max-w-[250px] rounded-xl" /> :
                     ...
                     isFile ? (
                       <a href={msg.attachment_url!} target="_blank" rel="noopener noreferrer" download={msg.attachment_name || undefined}
                         className={`flex items-center gap-3 px-3 py-2 rounded-xl ${isMine ? "bg-black/10" : "bg-background/40"}`}>
                         <div className={`size-10 rounded-lg flex items-center justify-center ${isMine ? "bg-black/20" : "bg-gold/15"}`}>
                           <FileText className={`size-5 ${isMine ? "text-primary-foreground" : "text-gold"}`} />
                         </div>
                         <div className="flex-1 min-w-0">
                           <p className={`text-xs font-semibold truncate ${isMine ? "text-primary-foreground" : "text-champagne"}`}>{msg.attachment_name}</p>
                           <p className={`text-[10px] ${isMine ? "text-primary-foreground/70" : "text-muted-foreground"}`}>{formatBytes(msg.attachment_size)}</p>
                         </div>
                         <Download className={`size-4 ${isMine ? "text-primary-foreground" : "text-gold"}`} />
                       </a>
                     ) :
                     isPoll ? (
                       <GroupPollBlock
                         pollId={msg.poll_id!}
                         isMine={isMine}
                         canModerate={isAdmin || msg.sender_id === user?.id}
                       />
                     ) :
                     body}
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-3 bg-background/80 backdrop-blur-xl border-t border-border/30">
        {showAttachMenu && (
          <div className="mb-2 grid grid-cols-3 gap-2 p-2 rounded-2xl bg-surface border border-border/40">
            <button onClick={() => { setShowAttachMenu(false); fileRef.current?.click(); }} className="flex flex-col items-center gap-1 py-3 rounded-xl hover:bg-background/60">
              <Image className="size-5 text-gold" />
              <span className="text-[10px] text-foreground">Photo / Video</span>
            </button>
            <button onClick={() => { setShowAttachMenu(false); docFileRef.current?.click(); }} className="flex flex-col items-center gap-1 py-3 rounded-xl hover:bg-background/60">
              <Paperclip className="size-5 text-gold" />
              <span className="text-[10px] text-foreground">File</span>
            </button>
            <button onClick={() => { setShowAttachMenu(false); setShowPollCreator(true); }} className="flex flex-col items-center gap-1 py-3 rounded-xl hover:bg-background/60">
              <BarChart3 className="size-5 text-gold" />
              <span className="text-[10px] text-foreground">Poll</span>
            </button>
          </div>
        )}
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={uploading}
            aria-label="Attach"
            className={`size-9 rounded-full flex items-center justify-center text-muted-foreground hover:text-gold transition-colors ${showAttachMenu ? "bg-gold/10 text-gold rotate-45" : ""}`}
          >
            <Plus className="size-5" />
          </button>
          <input ref={fileRef} type="file" accept="image/*,video/*" onChange={handleMedia} className="hidden" />
          <input ref={docFileRef} type="file" onChange={handleFile} className="hidden" />
          <input type="text" placeholder={uploading ? "Uploading…" : "Message • try @JagxAI or /imagine …"} value={input}
            onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()}
            disabled={uploading}
            className="flex-1 px-4 py-3 rounded-xl bg-surface border border-border text-sm text-foreground placeholder:text-muted-foreground outline-none disabled:opacity-60" />
          <button onClick={() => sendMessage()} disabled={!input.trim() || aiBusy || uploading} className="size-11 rounded-xl gold-gradient flex items-center justify-center text-primary-foreground disabled:opacity-50">
          ...
          <div className="p-4 space-y-3">
            {members.map(m => (
              <div key={m.user_id} className="flex items-center gap-3">
                ...
                {isAdmin && m.user_id !== user?.id && m.user_id !== groupInfo?.creator_id && (
                  <button
                    onClick={() => removeMember(m.user_id, m.username || "User")}
                    aria-label={`Remove ${m.username || "user"}`}
                    className="ml-2 size-8 rounded-full flex items-center justify-center text-destructive hover:bg-destructive/10"
                  >
